...
class UpSet:
    ...
    def _label_sizes(self, ax, rects, where):
        ...
        if where == "right":
            # VENDOR FIX: np.diff returns a length-1 array; .item() makes margin a scalar
            # so the ax.text coordinate is scalar.  numpy 2 raises on float() of a
            # non-0-d array, which crashed the original.  See vendored/upsetplot/NOTICE.md.
            ...
            # VENDOR FIX: see the "right" branch above.
            margin = 0.01 * abs(np.diff(ax.get_xlim())).item()
            for rect in rects:
                width = rect.get_width() + rect.get_x()
                ax.text(
