...
def test_matches_user_gwas_cholesky_on_small_slice():
    ...
    synthetic data, and assert our kernel matches for both F~SNP and R~SNP.
    """
    ...
    # Cholesky subtraction model in lavaan syntax. F = common factor (defined
    # by reference trait t2), R = remainder unique to composite trait t1.
    model = (
        "F =~ NA*t1 + t2\n"
        "R =~ NA*t1\n"
        "F ~ SNP\n"
    ...
    with localconverter(ro.default_converter + pandas2ri.converter):
        r_f_df = ro.conversion.get_conversion().rpy2py(r_result.rx2(1))
        r_r_df = ro.conversion.get_conversion().rpy2py(r_result.rx2(2))

    # userGWAS names the sandwich SE column "SE" (not "se_c" like commonfactorGWAS).
    r_est_F = np.asarray(r_f_df["est"], dtype=float)
    r_se_c_F = np.asarray(r_f_df["SE"], dtype=float)
    r_est_R = np.asarray(r_r_df["est"], dtype=float)
    r_se_c_R = np.asarray(r_r_df["SE"], dtype=float)
    ...
    # ---- Compare ----
    ok = ~py.fail & np.isfinite(r_est_F) & np.isfinite(r_est_R)
    ...
    np.testing.assert_allclose(py.beta_F[ok], r_est_F[ok], atol=5e-5, rtol=2e-3)
    np.testing.assert_allclose(py.beta_R[ok], r_est_R[ok], atol=5e-5, rtol=2e-3)
    np.testing.assert_allclose(py.se_c_F[ok], r_se_c_F[ok], atol=5e-4, rtol=5e-2)
    np.testing.assert_allclose(py.se_c_R[ok], r_se_c_R[ok], atol=5e-4, rtol=5e-2)
