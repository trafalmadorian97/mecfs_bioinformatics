...
As might be expected given the low case number and noisy phenotype, there are no significant tissues.  Nevertheless, it is interesting to observe that the tissues with the lowest p-values (_Brain_Cortex_, etc.) roughly correspond to the significant tissues found when [MAGMA was run on DecodeME](../DecodeME/g_MAGMA_DecodeME_Analysis.md)
