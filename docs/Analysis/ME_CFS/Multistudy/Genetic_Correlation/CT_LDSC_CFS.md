...
_The columns are as follows: p1: study 1; p2: study 2; rg: genetic correlation; se: standard error; z: z-score. p: p value. gcov_int: CT-LDSC intercept. 



The first observation[^numerical] is that the various DecodeME subset-GWAS are highly genetically correlated[^overlap]. Particularly notable is the high genetical correlation between male and female ME/CFS patients; and between ME/CFS patients who do and do not-report an infectious onset. The correlation between male and female patients suggest that the underlying genetic mechanism driving ME/CFS is likely the same in both sexes.  The correlation between infectious-onset and non-infectious onset patients suggests that either: a) the pathological process driving ME/CFS is the same regardless of whether it is triggered by an infectious agent or b) self-report of infectious onset is unreliable.
