...
1.  Select 15 [UK biobank](../../../../../Data_Sources/UKBB.md) traits with [genetic correlations](../../../../../Bioinformatics_Concepts/Genetic_Correlation.md) less than 0.2.
...
4.  Use these cross-trait averaged heritability enrichments to define a Bayesian prior: the prior probability that a variant is causal is proportional to the heritability enrichment implied by its functional annotations.
5. For robustness, modify the prior by limiting its dynamic range and binning in.
...
These results show that while with a uniform prior SUSIE produces a rather diffuse credible set, with the PolyFun prior the credible set is significantly more concentrated on a small number of variants.  The top PolyFun SUSIE variant is the SNP **chr1:173855298:A:T**.  PolyFun assigned a high prior weight to this variant due to annotations in the _coding_  and _conserved_ families.  The second PolyFun SUSIE variant is the deletion **chr1:173838788:TG:T**.  PolyFun assigned high prior weight to this variant due to annotations in the _promoter_or_enhancer_ and _histone_marks_ families.
...
- **chr1:173838788:TG:T** is a variant in an evolutionarily ancient promoter (_Ancient_Sequence_Age_Human_Promoter_common_) for the ZBTB37 gene.
