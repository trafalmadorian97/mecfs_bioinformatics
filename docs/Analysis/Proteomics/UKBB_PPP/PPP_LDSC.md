...
It is interesting to note that the all-variant and cis-excluded heritabilities are generally very similar.  This suggests that, consistent with LDSC model, even when it is provided with summary statistics containing highly localized signal spikes, LDSC will mostly ignore these spikes and compute heritability from a diffuse polygenic signal
