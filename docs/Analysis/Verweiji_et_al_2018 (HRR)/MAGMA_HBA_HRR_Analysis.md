...
I applied MAGMA to the hear rate recovery GWAS of Verweiji et al.[@verweij2018genetic] using scRNAseq data from the [Human Brain Atlas](../../Data_Sources/HBA_scRNAseq.md) as a reference.
...
This result is striking.  Notice that the independently significant cell-type cluster is concentrated in the [medulla oblongata](https://en.wikipedia.org/wiki/Medulla_oblongata), a region of the brainstem. The brainstem is a center of autonomic nervous system control. Given that the autonomic nervous system is key to the control of heart rate, it makes physiologic sense that genes associated with heart rate recovery after exercise should be expressed in an autonomic control center.