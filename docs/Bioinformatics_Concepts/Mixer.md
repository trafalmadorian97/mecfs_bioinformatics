...
MiXeR[@holland2020beyond;@frei2019bivariate] is a family of parametric probabilistic models for the distribution of GWAS effect sizes.  Statistical geneticists often fit MiXeR models to GWAS of traits of interest, then inspect the resulting model parameters to learn about the genetic architecture of these traits.
...
Univariate MiXeR[@holland2020beyond] is a model for the distribution of effect sizes in GWAS of a single trait.
...
- $y\in\mathbb{R}^N$ is the vector of phenotypes of study participants.  Assume $y$ is normalized to have mean 0 and sample variance 1.
- $\beta_i\in\mathbb{R}$ is the true causal regression coefficient of the $i$th genetic variant.
- $g_i \in\mathbb{R}^N$ is the vector of genotypes of study participants at the $i$th genetic variant.  Assume $g_i$ is normalized to have mean 0.
- $\epsilon\in\mathbb{R}^N$ is the vector of environmental and other effects.

Furthermore,
 
- Define $H_i = \frac{g_i^T g}{N}$, the sample variance of genetic variant $i$.
- Define 
...
$$
\begin{align}
\beta_i \sim \pi_1 \mathcal{N}(0, \sigma^2_\beta) + (1-\pi_1) \mathcal{N}(0,0) \label{uni_mix_core}
\end{align}
$$
...
By allowing some variants to have no effect, $(\ref{uni_mix_core})$ is more general than the commonly infinitesimal genomic model, which assumes that all variants affect the phenotype.
...
- Write this model as a probability distribution over $z$-scores of genetic variants.  This is useful because GWAS summary statistics are either expressed as $z$-scores, or are easily transformed into $z$-scores
- Derive an efficient algorithm to fit the MiXeR model by maximum likelihood.

We begin by deriving an expression for $\hat{\beta}_i$, the marginal GWAS regression coefficient of variant $i$.


$$
\begin{align}
\hat{\beta}_i&=\frac{y^Tg_i}{g_i^t g_i} \text{ (OLS coefficient definition)}\\
&= \frac{y^Tg_i}{N H_i}\\
