function (covstruc, SNPs, SNPSE = FALSE, parallel = FALSE, cores = NULL, 
    GC = "standard") 
{
    ...
    if (parallel == TRUE) {
        ...
        V_Full_List <- mclapply(X = 1:f, FUN = function(X) {
            ...
            if (GC == "conserv") {
                for (p in 1:nrow(coords)) {
                  ...
                  y <- coords[p, 2]
                  if (x != y) {
                    V_SNP[x, y] <- (SE_SNP[i, y] * SE_SNP[i, 
                      x] * I_LD[x, y] * I_LD[x, x] * I_LD[y, 
                      y] * varSNP[i]^2)
                  }
                  if (x == y) {
                    V_SNP[x, x] <- (SE_SNP[i, x] * I_LD[x, x] * 
                      varSNP[i])^2
                  }
                }
            }
            if (GC == "standard") {
                for (p in 1:nrow(coords)) {
                  ...
                  if (x != y) {
                    V_SNP[x, y] <- (SE_SNP[i, y] * SE_SNP[i, 
                      x] * I_LD[x, y] * sqrt(I_LD[x, x]) * sqrt(I_LD[y, 
                      y]) * varSNP[i]^2)
                  }
                  if (x == y) {
                    V_SNP[x, x] <- (SE_SNP[i, x] * sqrt(I_LD[x, 
                      x]) * varSNP[i])^2
                  }
                }
            }
            ...
            k2 <- nrow(V_Full)
            smooth2 <- ifelse(eigen(V_Full)$values[k2] <= 0, 
        ...
        S_Full_List <- mclapply(X = 1:f, FUN = function(X) {
            ...
            ks <- nrow(S_Full)
            smooth1 <- ifelse(eigen(S_Full)$values[ks] <= 0, 
    ...
    print(time_all[3])
    return(Output <- list(V_Full = V_Full_List, S_Full = S_Full_List, 
        RS = SNPs2))
}
