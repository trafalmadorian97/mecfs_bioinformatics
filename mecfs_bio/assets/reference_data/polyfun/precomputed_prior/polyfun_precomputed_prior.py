...
def create_prior_col_pipe(q:int) -> DataProcessingPipe:
    ...
    prior_with_floor_pipe = ExprPipe(
        narwhals.when(
            narwhals.col(POLYFUN_H_WEIGHT_COL)<=floor
        ).then(floor).otherwise(
            narwhals.col(POLYFUN_H_WEIGHT_COL)
...
COMBINED_POLYFUN_PRECOMPUTED_HERITABILITY_WEIGHTS= ConcatFramesTask.create(
    ...
    frames_tasks=[POLYFUN_PRECOMPUTED_HERITABILITY_WEIGHT_CHR_1_7,
