...
retaining strand-ambiguous palindromic variants (i.e. built from the keep-ambiguous rsid-assignment chain).

This is used by the S-LDSC discrepancy experiment, where we want to feed an input that still contains
palindromic SNPs into S-LDSC and optionally filter them at the S-LDSC task level, so that palindrome
handling can be varied as an experimental factor.  See `mecfs_bio.analysis.sldsc_discrepancy_experiment`.
"""
...
DECODE_ME_GWAS_1_37_ANNOVAR_RSIDS_SUMSTATS_KEEP_AMBIGUOUS = GWASLabCreateSumstatsTask(
    df_source_task=DECODE_ME_GWAS_1_37_ANNOVAR_DBSNP150_RSID_ASSIGNED_KEEP_AMBIGUOUS.join_task,
    target_asset_id=AssetId(
        "decode_me_gwas_1_37_sumstats_rsids_from_annovar_keep_ambiguous"
    ),
    basic_check=True,
    genome_build="infer",
    liftover_to="19",
