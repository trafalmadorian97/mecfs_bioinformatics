...
DECODE_ME_S_LDSC_ALL_TABLES_FDR = [
...
DECODE_ME_S_LDSC_ALL_TABLES_FDR_COMBINED = CopyFilesIntoDirectoryTask(
    sources=[
        CopySource(item, suffix=".csv") for item in DECODE_ME_S_LDSC_ALL_TABLES_FDR
    ],
    meta=ResultDirectoryMeta(
        id="decode_me_sldsc_fdr",
        trait="ME_CFS",
        project="DecodeME",
...
DECODE_ME_S_LDSC_ALL_TABLES_FDR_ZIP = ZipDirTask.create(
    source_dir_task=DECODE_ME_S_LDSC_ALL_TABLES_FDR_COMBINED,
    asset_id="decode_me_sldsc_fdr_archive",
)
