...
from mecfs_bio.build_system.meta.read_spec.read_dataframe import scan_dataframe_asset, scan_dataframe
...
def load_contents(set_source:SetSource, fetch: Fetch)->list[str]:
    if isinstance(set_source, FileSetSource):
        ...
        df =set_source.pipe.process(scan_dataframe_asset(asset, meta=set_source.task.meta)).collect().to_pandas()
        return df[set_source.col_name].tolist()
    elif isinstance(set_source, DirSetSource):
        asset= fetch(set_source.task.asset_id)
        assert isinstance(asset, DirectoryAsset)
        df = set_source.pipe.process(scan_dataframe(path=asset.path/set_source.file_in_dir,spec=set_source.read_spec)).collect().to_pandas()
        return df[set_source.col_name].tolist()
    else:
