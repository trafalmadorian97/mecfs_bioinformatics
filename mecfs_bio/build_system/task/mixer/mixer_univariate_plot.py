...
from mecfs_bio.build_system.task.mixer.mixer_univariate_combine import COMBINED_TEST_FILENAME_PREFIX
...
PLOT_TEST_OUTPUT_PREFIX = "trait_plot_test"
@frozen
class MixerUnivariatePlot(Task):
    _meta: Meta
    combine_task: Task
    trait_name: str
    @property
    ...
    def execute(self, scratch_dir: Path, fetch: Fetch, wf: WF) -> Asset:
        ...
        plots_mounts = {scratch_dir.resolve(): _CONTAINER_PLOT_DIR,
        ...
        invoke_mixer_figures(
            args=[
                "one",
                "--json",
                str(_CONTAINER_COMBINED_DIR/(COMBINED_TEST_FILENAME_PREFIX+".json")),
                "--out",
                str(_CONTAINER_PLOT_DIR/ (PLOT_TEST_OUTPUT_PREFIX)),
                "--statistic",
                "mean std",
                "--ext",
                "png",
    ...
    def create(cls,
        ...
        if isinstance(source_meta, ResultDirectoryMeta):
            meta=GWASPlotDirectoryMeta(
                ...
                id=AssetId(asset_id),
                sub_dir=PurePath("analysis")/"mixer_plots"
            )
