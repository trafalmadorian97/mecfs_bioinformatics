...
class PrepareSpecificityCepo(Task):
    ...
    CEPO is based on differential stability: a gene is considered specific for a cell type if its expression in stable in that
    ...
    def execute(self, scratch_dir: Path, fetch: Fetch, wf: WF) -> Asset:
        ...
        df = scan_dataframe_asset(long_count_df_asset, meta=self.long_count_df_task.meta)
        df = self.pipe.process(df)
        ...
        df = _compute_stability(
            df=df,num_genes=num_genes, )
        df = _compute_differential_stability(df=df,
                                             cell_type_col=self.cell_type_col,)
...
def _compute_inv_coef_prop_nonzero(
    ...
    df= df.group_by(
        ...
        (((narwhals.col(count_col)>0).sum())/(
            narwhals.col(count_col).len()
        )).alias(_count_prop_nonzero),
    )
    df = df.with_columns(
        (narwhals.col(_count_std)/(narwhals.col(_count_mean)+epsilon)).alias(_inv_coef)
    )
    return df


def _compute_ranks(df: narwhals.LazyFrame,
                   cell_type_col: str,
                   ) -> narwhals.LazyFrame:
    return df.with_columns(
            narwhals.col(_inv_coef).rank("max").over(cell_type_col,order_by=_inv_coef,).alias(_inv_coef_rank),
            narwhals.col(_count_prop_nonzero).rank("max").over(cell_type_col,order_by=_count_prop_nonzero).alias(_nonzero_rank),
    )


def _compute_stability(df: narwhals.LazyFrame, num_genes:int)-> narwhals.LazyFrame:
    return df.with_columns(
        (1- (narwhals.col(_inv_coef_rank) +narwhals.col(_count_prop_nonzero)  )/(2*num_genes) ).alias(
            _stability
        )
    )

def _compute_differential_stability(df: narwhals.LazyFrame, cell_type_col: str)-> narwhals.LazyFrame:
    return df.with_columns(
        (narwhals.col(_stability) - narwhals.col(_stability).mean().over(cell_type_col)).alias(
            DIFFERENTIAL_STABILITY
        )
    )