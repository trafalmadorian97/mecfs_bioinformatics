"""
Task to run the low-peak-memory POPs reimplementation against MAGMA gene-level
results, producing per-gene priority scores.

This is a drop-in alternative to PopsRunTask that invokes our streaming
kernel-ridge script (pops_lowmem.py) instead of stock pops.py. It is
model-identical to stock POPs (validated to near-machine precision) but its peak
memory is independent of the number of selected features, so it runs high-feature
traits (for example uncapped IBD) that OOM the box under stock POPs. It consumes
the same munged feature directory and MAGMA gene-analysis DirectoryAsset and writes
the same <stem>.preds, <stem>.coefs, and <stem>.marginals files.
"""
...
from mecfs_bio.build_system.task.pops.pops_run_task import DEFAULT_POPS_SUB_DIR
...
@frozen
class PopsLowMemRunTask(Task):
    """Fit the POPs model against MAGMA gene-level results using the low-peak-memory
    streaming kernel-ridge implementation."""
    ...
    def execute(self, scratch_dir: Path, fetch: Fetch, wf: WF) -> Asset:
        ...
        invoke_pops_lowmem_script(source_dir, args)
