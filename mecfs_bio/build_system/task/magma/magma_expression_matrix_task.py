...
from mecfs_bio.constants.magma_constants import MAGMA_GENE_COL, MAGMA_P_COLUMN
...
# class plot_config:
#     include_extra_gene_annotations:bool=
...
class ExpressionMatrixClusterMapTask(Task):
    ...
    def execute(self, scratch_dir: Path, fetch: Fetch, wf: WF) -> Asset:
        ...
        gene_cluster_matrix = gene_cluster_matrix_raw.copy()
        # gene_cluster_matrix =gene_cluster_matrix.set_index(self.gene_col_name)
        gene_cluster_matrix = gene_cluster_matrix.merge(gene_info_df, left_on=self.gene_col_name, right_on=MAGMA_GENE_COL).drop(columns=[self.gene_col_name, MAGMA_GENE_COL]).set_index(GENE_LABEL_COL)


        gene_cluster_matrix = gene_cluster_matrix.loc[:,gene_cluster_matrix.columns.isin(cluster_list)]




        result = sns.clustermap(
            data=gene_cluster_matrix,z_score=0
        )
        result.savefig(
...
def retrieve_gene_list_and_optional_annotations(
    ...
    if isinstance(gene_select_mode, TopGenesMode):
        ...
        sorted_filtered  =gene_df.sort_values(by=MAGMA_P_COLUMN)[:gene_select_mode.num_genes].loc[:,[ MAGMA_GENE_COL, MAGMA_P_COLUMN ]]
        gene_label_list = [f"{nm} (p={p_val})" for nm,p_val in zip(sorted_filtered[MAGMA_GENE_COL], sorted_filtered[MAGMA_P_COLUMN])]
        sorted_filtered[GENE_LABEL_COL] = gene_label_list
        return sorted_filtered.loc[:,[MAGMA_GENE_COL, GENE_LABEL_COL]]
