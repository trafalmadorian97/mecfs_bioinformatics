...
"Heritability enrichment of specifically expressed genes identifies disease-relevant tissues and cell types"
    ...
    def deps(self) -> list["Task"]:
        return [self.source_sumstats_task, self.ref_ld_chr_cts_task,self.ref_ld_chr_task, self.w_ld_chr_task ]
    ...
    def execute(self, scratch_dir: Path, fetch: Fetch, wf: WF) -> Asset:
        ...
        ref_ld_chr_cts_index_path = ref_ld_chr_cts_asset.path / self.ref_ld_chr_cts_index_filename
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            new_loc_chr_cts_index_path = new_loc_chr_cts/ self.ref_ld_chr_cts_index_filename
            ...
            # we need to prepend a path to al paths in the index file
            prepend_path_to_ldcts_file(new_loc_chr_cts_index_path,prefix_path=new_loc_chr_cts, output_path=new_loc_chr_cts_index_path)
            import pdb; pdb.set_trace()

            ref_ld_chr_asset = fetch(self._ref_ld_chr_id )
            assert isinstance(ref_ld_chr_asset, DirectoryAsset)
            ref_ld_chr_full_pattern = ref_ld_chr_asset.path / self.ref_ld_chr_inner_pattern
            ...
            logger.debug(
                f"cell type specific s-LDSC results: \n \n {ldsc_h2_cts}"
            )
...
class LDCTSFileEntry:
    ...
    def to_text(self) ->str:
        return  f"{self.label}\t{self.target_gene_data_path},{self.control_gene_data_path}"
