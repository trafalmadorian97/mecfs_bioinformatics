...
from mecfs_bio.build_system.task.fdr_multiple_testing_table_task import MultipleTestingTableTask, Method
...
class SLDSCTaskGenerator:
    ...
    def get_terminal_tasks(self) -> Sequence[Task]:
        return [
            item.terminal_task() for item in self.partitioned_tasks.values()
        ]
    ...
    def create(cls,
        ...
        for entry in partitioned_entries:
            cell_task=CellAnalysisByLDSCTask.create(
                asset_id=base_name+"_" +entry.entry_name + "_cell_analysis_by_s_ldsc",
            ...
            multiple_testing_task = MultipleTestingTableTask.create_from_result_table_task(
                asset_id=base_name+"_"+ entry.entry_name + "_multiple_testing_correction",
