...
# The European (discovery) cohort: the randomly selected baseline sub-cohort (n = 34,557),
# European ancestry only (see regenerate_ppp_manifest.py). Its LD structure matches the
# European HapMap3 LD-score reference used downstream, so it is the default for the LDSC
# pipeline.
...
def generate_ppp_slim_protein_tasks(
    index_task: Task,
    index_name: str,
    manifest_path: Path = EUR_DISCOVERY_PPP_MANIFEST_PATH,
    ...
    index_name: a short label for the (index, cohort) combination (e.g. hapmap_3). It
        namespaces both the asset ids and the on-disk paths, so the same protein aligned
        onto a different index -- or drawn from a different cohort -- produces distinct,
        non-colliding assets.
    manifest_path: the committed per-cohort manifest; defaults to the European (discovery)
        cohort.
...
def generate_ppp_sample_size_task(
    ...
    this takes no index. Defaults to the European (discovery) cohort manifest."""
    manifest = pl.read_csv(manifest_path)
    protein_refs = tuple(
        PppProteinRef(
