...
# Default 'baseline' partitioned-model reference (the latest version we use by default).
DEFAULT_BASELINE_REF_LD_CHR_TASK = BASE_MODEL_PARTITIONED_LD_SCORES_EXTRACTED
DEFAULT_BASELINE_REF_LD_CHR_INNER_DIRNAME = "baseline_v1.2/baseline.@"
...
def standard_sldsc_task_generator(
    ...
    ref_ld_chr_task: Task = DEFAULT_BASELINE_REF_LD_CHR_TASK,
    ref_ld_chr_inner_dirname: str = DEFAULT_BASELINE_REF_LD_CHR_INNER_DIRNAME,
    filter_settings: FilterSettings | None = None,
    build: GenomeBuild = "19",
    partitioned_entry_names: set[str] | None = None,
) -> SLDSCTaskGenerator:
    """
    ...
    if partitioned_entry_names is not None:
        all_partitioned_entries = [
            entry
            for entry in all_partitioned_entries
            if entry.entry_name in partitioned_entry_names
        ]
    result = SLDSCTaskGenerator.create(
        ...
        partitioned_entries=all_partitioned_entries,
