...
def get_broad_ukbb_ld_matrix_file_info()-> pd.DataFrame:
    df = pd.read_csv(BROAD_UKBB_FILE_LIST, sep=r"\s+",
                     names=[
...
def get_optimal_ukbb_ld_interval(
    chrom:int,
    pos: int
    )->GenomicInterval:
    broad_ld_df = get_broad_ukbb_ld_matrix_file_info()

    valid_intervals_df = broad_ld_df.loc[ (broad_ld_df["chrom"]==chrom)
    &(broad_ld_df["start"]<=pos) &(broad_ld_df["end"]>=pos)
    ]
    if len(valid_intervals_df)==0:
        raise ValueError(f"Could not find a broad UKKBB LD matrix for chromosome {chrom}, position {pos}")

    valid_intervals_df["start_dist"] = (pos - valid_intervals_df["start"])
    valid_intervals_df["end_dist"] = (valid_intervals_df["end"] - pos)
    valid_intervals_df["centrality"] = (valid_intervals_df.loc[:,["start_dist","end_dist"]]).min(axis=1)
    valid_intervals_df=valid_intervals_df.sort_values(by="centrality", ascending=False)
...
def get_genomic_interval_ld_labels_url(interval:GenomicInterval)->str:
    return "https://broad-alkesgroup-ukbb-ld.s3.amazonaws.com/UKBB_LD/" + get_genomic_interval_stem_name(interval) + ".gz"

def get_genomic_interval_ld_matrix_url(interval:GenomicInterval) -> str:
    return "https://broad-alkesgroup-ukbb-ld.s3.amazonaws.com/UKBB_LD/" + get_genomic_interval_stem_name(interval) + ".npz"
...
def get_ld_labels_and_matrix_task_for_chrom_pos_build_37(chrom:int, pos:int)-> tuple[Task, Task]:
    interval = get_optimal_ukbb_ld_interval(chrom=chrom, pos=pos)
    return get_ld_labels_and_matrix_task_for_genomic_interval_build_37(interval=interval)

def get_ld_labels_and_matrix_task_for_genomic_interval_build_37(interval:GenomicInterval):
    return (
